...
class StepModelCheckpoint(Callback):
    ...
    def __init__(self, filepath, verbose=0, monitor='val_loss',
                 save_best_only=False, save_weights_only=False, mode='auto', xlen=1,
                 period=1, start_step=10000, val_batch_size=None, model=None):
        super(StepModelCheckpoint, self).__init__()
            ...
            if self.steps_since_last_save >= self.period:
                ...
                if self.save_best_only:
                    ...
                    if self.xlen == 1:
                        val_x = val_data[0]
                        val_y = val_data[1]
                        val_sample_weights = val_data[2]
                    else:
                        val_x = val_data[0: self.xlen]
                        val_y = val_data[self.xlen]
                        val_sample_weights = val_data[self.xlen+1]
                    if self.val_batch_size is not None:
                        batch_size = self.val_batch_size
                    else:
                        batch_size = logs.get('size')
...
class StepF1ModelCheckpoint(Callback):
    """Save the model after specified interval of steps with f1 monitor.
